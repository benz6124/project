...
angular.module('bootstrapLightbox').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('lightbox.html',
    "<div class=modal-body ng-swipe-left=Lightbox.nextImage() ng-swipe-right=Lightbox.prevImage()><div class=lightbox-nav><a class=close style='font-size:40px' ng-click=$dismiss()>×</a><div ng-if=\"Lightbox.images.length > 1\"><a href='' style=margin-left:0px ng-click=Lightbox.prevImage()><span class='glyphicon glyphicon-circle-arrow-left'></span>  รูปก่อนหน้า                               </a> <a class='pull-right' style='margin-right:20px' href='' ng-click=Lightbox.nextImage()>                           รูปถัดไป <i class='glyphicon glyphicon-circle-arrow-right'></i></a></div></div><div class=lightbox-image-container><div class=lightbox-image-caption><span>{{Lightbox.imageCaption}}</span></div><img ng-if=!Lightbox.isVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}}><div ng-if=Lightbox.isVideo(Lightbox.image) class=\"embed-responsive embed-responsive-16by9\"><video ng-if=!Lightbox.isSharedVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}} controls autoplay></video><embed-video ng-if=Lightbox.isSharedVideo(Lightbox.image) lightbox-src={{Lightbox.imageUrl}} ng-href={{Lightbox.imageUrl}} iframe-id=lightbox-video class=embed-responsive-item><a ng-href={{Lightbox.imageUrl}}>Watch video</a></embed-video></div></div></div>"
  );
